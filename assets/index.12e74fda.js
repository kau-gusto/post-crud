...
`;function se(){const[e,t]=_(),[,n]=C(),[u,o]=c.exports.useState(0),[a,f]=v(),[i,g]=c.exports.useState(()=>{var h;const s=parseInt((h=e.get("page"))!=null?h:"1");return isNaN(s)?1:s}),l=5,d=c.exports.useCallback(()=>{const s=a.length/l;return s>Math.floor(s)?Math.floor(s)+1:Math.floor(s)},[a,l]);function b(){g(i+1)}function F(){g(i-1)}function L(s){fetch(`https://jsonplaceholder.typicode.com/posts/${s}`,{method:"DELETE"}).then(()=>{f(a.filter(h=>h.id!==s))}).catch(()=>{alert("N\xE3o foi possivel remover o post")})}return c.exports.useEffect(()=>{var h;n(0);const s=parseInt((h=e.get("page"))!=null?h:"1");i!==s&&t({page:i.toString()})},[i]),c.exports.useEffect(()=>{var h;const s=parseInt((h=e.get("page"))!=null?h:"1");isNaN(s)||i!==s&&g(s)},[e]),c.exports.useEffect(()=>{const s=d();o(s),g(s!==0?i>s?s:i:1)},[a]),p(I,{children:[r(oe,{children:a.filter((s,h)=>h>=(i-1)*l&&h<i*l).map(s=>r($,{post:s,onDelete:L},s.id))}),p(ne,{children:[i>1?r("button",{onClick:F,children:"<"}):null,i,i<u?r("button",{onClick:b,children:">"}):null]})]})}function ae(){var f;const t=(f=A().id)!=null?f:"0",[n,u]=v(),[o]=n.filter(i=>i.id==parseInt(t));function a(i){fetch(`https://jsonplaceholder.typicode.com/posts/${i}`,{method:"DELETE"}).then(()=>{u(n.filter(g=>g.id!==i))}).catch(()=>{alert("N\xE3o foi possivel remover o post")})}return c.exports.useMemo(()=>o?r($,{post:o,onDelete:a}):r(I,{}),[o])}q.render(p(H.StrictMode,{children:[r(V,{}),r(K,{children:p(J,{basename:"/post-crud/",children:[r(Q,{}),r(X,{children:p(W,{children:[r(k,{path:"/",element:r(se,{})}),r(k,{path:"/post/:id",element:r(ae,{})})]})})]})})]}),document.getElementById("root"));
